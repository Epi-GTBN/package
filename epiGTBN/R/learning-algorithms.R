...
greedy.search = function(x, start = NULL, whitelist = NULL, blacklist = NULL,
    score = "aic", heuristic = "gtbn2", expand, optimized = FALSE, debug = FALSE) {
  ...
  if (heuristic == "gtbn2") {
    ...
    res = gtbn.2(x = x, start = start, whitelist = whitelist,
  ...
  if (heuristic == "gtbn3") {
    ...
    res = gtbn.3(x = x, start = start, whitelist = whitelist,
      blacklist = blacklist, score = score, extra.args = extra.args,
      restart = restart, perturb = perturb, max.iter = max.iter,
      maxp = maxp, optimized = optimized, debug = debug)
...
hybrid.search = function(x, whitelist = NULL, blacklist = NULL,
    restrict = "mmpc", maximize = NULL, restrict.args = list(), score = NULL,
  ...
  # method = ifelse((restrict == "mmpc") && (maximize == "tabu"), "mmhc", "rsmax2")

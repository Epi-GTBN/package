...
SEXP start, cur_n, cur_p, cur_c, temp, name_nodes, name_cached;
  ...
  if (!strcmp(op, "set")) {
    ...
    cur_n = getListElement(start, to);
    cur_p = getListElement(cur_n, "parents");
    //Rprintf("$gy in score_delta_helper, to: %s.\n",to);
    //Rprintf("$gy in score_delta_helper, length(cur_n): %d.\n",length(cur_n));
    //Rprintf("$gy in score_delta_helper, length(cur_p): %d.\n",length(cur_p));
    PROTECT(parents_to = allocVector(STRSXP, length(cur_p) - 1));
...
SEXP delta, fake, new_score, try, to_update;
  ...
  //Rprintf("$gy in score_delta_decomposable: before new_score = allocVector(REALSXP, length(to_update)).\n");
  PROTECT(new_score = allocVector(REALSXP, length(to_update)));
  //Rprintf("$gy in score_delta_decomposable: after new_score = allocVector(REALSXP, length(to_update)).\n");
  new = REAL(new_score);
  ...
  if (length(new_score) == 1){
    //Rprintf("$gy in score_delta_decomposable: length(new_score) == 1 scenario.\n");
